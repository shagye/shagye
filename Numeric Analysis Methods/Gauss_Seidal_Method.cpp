...
void SoruUc::baslat()
{
    ...
    if(secenek=="1")
    {
		...
		for(int i=1;i<=iterasyon;i++){
			//formul tanimlama
				...
				x_deger[1][i]=(d_matris[1]-((k[1][0]*x_deger[0][i])+(k[1][2]*x_deger[2][i-1])))/k[1][1];
				x_deger[2][i]=(d_matris[2]-((k[2][0]*x_deger[0][i])+(k[2][1]*x_deger[1][i])))/k[2][2];
				
	    }
		cout<<endl;
		cout<<"\nHESAPLAMALAR\n";
		for(int i =0;i<iterasyon;i++){
			cout<<"x("<<i+1<<".iterasyon)\t"<<setprecision(9)<<x_deger[0][i+1]<<"|"<<x_deger[1][i+1]<<"|"<<x_deger[2][i+1]<<endl<<endl;
	...
	{
		iterasyon=2;
		k[0][0]=4; k[0][1]=-1; k[0][2]=1;
		k[1][0]=1; k[1][1]=6;  k[1][2]=2;
		k[2][0]=1; k[2][1]=2;  k[2][2]=-5;
		float x_deger[3][iterasyon];
		float d_matris[3];
		d_matris[0]=4;  d_matris[1]=9;   d_matris[2]=-2;
		x_deger[0][0]=1.1;	  x_deger[1][0]=1.7;    x_deger[2][0]=1.3;	
		for(int i=1;i<=iterasyon;i++){
			//formulun tanimlanmasi
				x_deger[0][i]=(d_matris[0]-((k[0][1]*x_deger[1][i-1])+(k[0][2]*x_deger[2][i-1])))/k[0][0];
				x_deger[1][i]=(d_matris[1]-((k[1][0]*x_deger[0][i])+(k[1][2]*x_deger[2][i-1])))/k[1][1];
				x_deger[2][i]=(d_matris[2]-((k[2][0]*x_deger[0][i])+(k[2][1]*x_deger[1][i])))/k[2][2];
